...
# Configuration
PATH_ROOT = utils.get_path()

parser = argparse.ArgumentParser(description="Script para procesar videos de YouTube.")

parser.add_argument("--mp3_name", help="Nombre del archivo mp3", required=True)

parser.add_argument("--disc_patter_uuid", type=str, default="b5a22823-f289-fe86-79ac-3963643f0298"  )

args = parser.parse_args()

UUID_PATTER_DISC = args.disc_patter_uuid
FILENAME = args.mp3_name

INPUT_FOLDER = PATH_ROOT+"/inputs/"
OUTPUT_FOLDER = PATH_ROOT+"/outputs/"+FILENAME

INPUT_AUDIO = INPUT_FOLDER+FILENAME
...
def find_input_file(input_file):
    ...
    formats = ["mp3", "mp4", "wav", "3gp", "aac", "flv", "m4a", "ogg", "webm", "mkv"]
    for fmt in formats:
...
def amplify_audio_audicty(output,amplify):
    output = utils.convert_path(output)
    amplify = utils.convert_path(amplify)

    pipi_audacity.audacity_amplify_compressor(output, amplify, "14","8")
...
amplify_audio_audicty(OUTPUT_AUDIO,AMPLIFIED_AUDIO)
